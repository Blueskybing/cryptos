 ...
 * 构造PEM编码
 * @param key
 * @param options {type:公钥传'public_key'，私钥传'private_key',scheme:传‘pkcs1’或‘pkcs8’.}
 * @returns {string}
 */
cryptos.pem = function (key, options) {
    ...
    options.scheme = options.scheme || 'pkcs8';
    options.type = options.scheme || 'private_key';
    var obj = config[options.scheme][options.type];
    // 构造PEM编码
